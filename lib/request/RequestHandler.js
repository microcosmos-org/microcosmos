...
export default class RequestHandler {
   ...
   */
  async handle() {
    if (this.isOptionsRequest) return void this.render.options();
  ...
  /**
   * @return {MicroActionStore|null} Maps ActionStore form Request
   * @private
   */
  get actionsStore() {
  ...
  actionNotFound(message) {
    if (process.env.NODE_ENV !== 'production') {
      ...
      this.render.json({ data: 'Somebody is lost' }, 404);
    }
  }
}
